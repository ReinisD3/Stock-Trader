                ...
                <div class="flex my-10 border-2 border-blue-700 rounded-sm p-5 bg-blue-50 justify-between items-center">
                    <div>
                        <span class="text-gray-500">Amount </span> <span
                            class="text-3xl">{{$trade->amount_bought}} </span> <span class="text-gray-500">/Stock Count</span>
                    </div>
                </div>
                <div class="flex my-10 border-2 border-blue-700 rounded-sm p-5 bg-blue-50 justify-between items-center">
                    <div>
                        <span class="text-gray-500">{{$company->profile()->getCurrency()}}</span> <span
                            class="text-3xl">{{$price}} </span> <span class="text-gray-500">/current Stock Value</span>
                    </div>
                </div>

                <h1 class="text-gray-700 font-bold tracking-wider">Sell Stock</h1>

                <form method="post" action="{{ route('stock.sell', $trade) }}"
                    ...
                    <label for="amount">Stock amount ({{$trade->amount_bought}}) </label>
                    <button type="submit"
