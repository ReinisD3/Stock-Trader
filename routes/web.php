...
Route::get('/user/profile', [UsersController::class, 'showProfile'])->middleware('auth')->name('user.profile');
Route::post('/user/deposit',[UsersController::class, 'deposit'])->middleware('auth')->name('user.deposit');
Route::get('user/trades',[UsersController::class, 'showTrades'])->middleware('auth')->name('user.trades');

Route::post('/stock/{companySymbol}/buy',[TradesController::class, 'buy'])->middleware('auth')->name('stock.buy');
Route::post('/stock/sell',[TradesController::class,'sell'])->middleware('auth')->name('stock.sell');
